...
class ParserModel(Model):
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        ce_loss = tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred)
        reg_loss = self.l_reg * tf.nn.l2_loss(self.W)
        loss = tf.reduce_mean(ce_loss + reg_loss)
        ### END YOUR CODE
        return loss
