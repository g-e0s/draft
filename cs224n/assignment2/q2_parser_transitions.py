...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        prediction = model.predict(minibatch)
        # Perform a parse step on each partial parse in the minibatch with its predicted transition
        for i, transition in enumerate(prediction):
        ...
        unfinished_parses = filter(lambda x: not (len(x.buffer) == 0 and len(x.stack) == 1), unfinished_parses)
