...
class AutoEncoder(nn.Module):
    def __init__(self, inp_size, hid_size):
        ...
        _layer_1_size = max(inp_size // 4, hid_size * 8)
        _layer_2_size = max(inp_size // 8, hid_size * 4)

        self.encoder = nn.Sequential(
            nn.Linear(inp_size, _layer_1_size),
            nn.ReLU(True),
            nn.Linear(_layer_1_size, _layer_2_size),
            nn.ReLU(True),
            nn.Linear(_layer_2_size, hid_size),
            nn.ReLU(True))
        self.decoder = nn.Sequential(
            nn.Linear(hid_size, _layer_2_size),
            nn.ReLU(True),
            nn.Linear(_layer_2_size, _layer_1_size),
            nn.ReLU(True),
            nn.Linear(_layer_1_size, inp_size),
            nn.ReLU(True))
    ...
    def loss_function(self, recon_x, x):
        ...
        :return: loss
        """
        return self.loss.forward(recon_x, x)
...
def train(model, optimizer, train_loader, test_loader):
    for epoch in range(10):
        ...
        print('=> Epoch: %s Average loss: %.5f' % (epoch, train_loss / len(train_loader.dataset)))
